...
public class Level2 {
	...
	/* TO DO 2: Retourner la somme des salaires de tous les employ�s (hint: mapToInt) */	
		long sum = employees.stream().mapToLong(e->e.getSalary()).sum();

		/* TO DO 3: Retourner la moyenne des salaires des employ�s dont le nom commence avec s */
	OptionalDouble average = employees.stream().filter(e->e.getName().startsWith("n")).mapToDouble(e->e.getSalary())
				.average();
